...
export default function ProjectTaskPage() {
  ...
  const stopTimer = async () => {
    ...
    try {
      ...
      console.log('Stopped timer. Spent seconds:', spentSeconds);

      await fetchTasks();

      // update local time
      const entries = await getTimeEntriesForTask(task.value);
      console.log('Fetched entries after stop:', entries);

      const totalSeconds = entries.reduce(
        (sum, e) => sum + Math.round(Number(e.hours_spent) * 3600),
        0
      );
      console.log('Calculated totalSeconds from entries:', totalSeconds);

      setTaskTimes((prev) => ({ ...prev, [task.value]: totalSeconds }));
  ...
  // Batch fetch times for filtered tasks to populate the dropdown
  useEffect(() => {
    if (!filteredTasks.length || !authToken) return;
    ...
    const fetchAllTimes = async () => {
      ...
      const chunks = chunk(filteredTasks, 5);
      ...
      for (const group of chunks) {
        ...
        await Promise.all(
          group.map(async (t) => {
            // Skip if we already have it (optional, but good for performance)
            if (taskTimes[t.id] !== undefined) return;

            try {
              const entries = await getTimeEntriesForTask(t.id);
              const totalSeconds = entries.reduce(
                (sum, e) => sum + Math.round(Number(e.hours_spent) * 3600),
                0
              );
              ...
              if (active) {
                setTaskTimes(prev => ({ ...prev, [t.id]: totalSeconds }));
              }
            } catch (err) {
              console.error(`Failed to fetch time for task ${t.id}`, err);
            }
          })
        );
      }
  ...
  }, [filteredTasks, authToken]); // dependency on filteredTasks ensures we fetch when project changes or tasks load
  ...
  const taskOption = useMemo(() => {
    return filteredTasks.map((t) => {
      ...
      //   localStorage.getItem(`timer-${user.id}-${t.id}`) || 0
      // );

      // const savedTime =
      //   t.savedTime !== undefined && t.savedTime !== null
      //     ? t.savedTime
      //     : localTime ?? 0;
      const localTime = taskTimes[t.id];
      const savedTime = localTime !== undefined ? localTime : Number(t.savedTime || 0);
      return {
        value: t.id,
  ...
  }, [filteredTasks, taskTimes, user?.id]);
    ...
    <div className="max-w-md mx-auto px-4 py-2 space-y-5 font-sans bg-white mt-2 mb-2 overflow-visible">
      ...
      <div className="flex justify-between items-center mt-1">
        ...
        <div className="flex items-center gap-3">
          <h2
            className={`text-xl font-bold font-mono w-[70px] text-center ${time >= 3600 ? "mr-7" : "mr-1"
              }`}
          >
            {formateTime(time)}
          </h2>
          ...
          <button
            ...
            disabled={!task}
            className={`px-4 py-1 rounded text-white font-semibold ${!task
              ? "bg-gray-300"
              : isRunning
        ...
        <div className="flex justify-between items-center">
          ...
          >
            ...
            <button
              ...
              disabled={isRunning}
              className={`p-2 border rounded ${isRunning
        ...
        <div ref={taskContainerRef} className="w-full">
          <Select
            ...
            onChange={async (selected) => {
              ...
              const entries = await getTimeEntriesForTask(selected.value);
              ...
              // Update local cache so the list label updates too
              setTaskTimes((prev) => ({ ...prev, [selected.value]: totalSeconds }));
            }}
